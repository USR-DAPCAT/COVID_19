...
##################################################################################################
#Hola Bogdan
#Es importante que usemos las mismas fechas que los británicos para distinguir la primera y la segunda ola
#Wave 1: data collected from 01/01/2020 – 30/06/2020 (ingres)

#Wave 2: data collected from 01/07/2020 – 31/03/2021 (ingres)

#es importante hacerlo igual, y solo con los diagnósticos seguros de diabetes
#gracias!
#Dídac
####################################################################################################
...
dt_cataleg2<-readxl::read_excel(conductor_codis2,col_types = "text")%>%select(cod,agr,agr2,agr2b,agr3,agr3b,agr3c,agr4,agr5,agr6,agr7,agr8,agrCVD,agrE09,agrE10,agrE11,agrCKD,agrE10_10)
...
#PATIENT.ID	=IDINGRESO son diferents!!!
...
#prova1<-dt_01%>% mutate(F_INGRESO.ADMISSION_D_ING.INPAT2=data.to.string(F_INGRESO.ADMISSION_D_ING.INPAT) %>% as.numeric())
#prova1<-prova1%>%arrange(F_INGRESO.ADMISSION_D_ING.INPAT2)
...
#[25.06.2021]
llista_vars<-c("FC.HR_ING.INPAT","TA_MAX_ING.INPAT","TA_MIN_ING.INPAT","TEMP_ING.INPAT","SAT_02_ING.INPAT")
...
#KKK<-TAULA_PLANA%>%select(idp,G_DIABETIC,DM1,DM2,FX.ADO,GRUP_DIABETIC_CAT)

# mirar tots els problemes de Salut :[[20_07_2020]]########################################
#i)
problemas_salut1c<-dt_diagnostics%>%select(cod)%>%group_by(cod)%>%dplyr::slice(1)%>%ungroup()
############################################################################################
#ii)
problemas_salut2c<-dt_plana_covid2%>%select(ID_ATC5,ATC5_NOMBRE.NAME)%>%group_by(ID_ATC5)%>%dplyr::slice(1)%>%ungroup()
############################################################################################
#iii)
problemas_salut3c<-dt_plana_covid2%>%select(ID_ATC7,ATC7_NOMBRE.NAME)%>%group_by(ID_ATC7)%>%dplyr::slice(1)%>%ungroup()
############################################################################################

write.csv(problemas_salut1c, file="Problemas_salut_2021.csv")
write.csv(problemas_salut2c, file="FARMACS_ATC5_2021.csv")
write.csv(problemas_salut3c, file="FARMACS_ATC7_2021.csv")
...
write.csv(TAULA_PLANA,"TAULA_PLANA3.csv")
