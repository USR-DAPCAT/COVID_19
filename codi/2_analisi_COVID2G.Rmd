...
&check; Model4:Death ~ Macrovascular disease + Age + Sex + FX.COMBI(ACEi/ARB_only  Statin_only  Both_ACEi/ARB_and_Statin  Neiher_ACEI/ARB/Statin(basal)) <br/>
&check; Model5:Death ~ Macrovascular disease + Age + Sex + FX.COMBI+HTA <br/>
&check; Model6:Death ~ Macrovascular disease + Age + Sex + FX.COMBI+HTA+CKD <br/>

&check; Model7:Death ~ Macrovascular disease + Age + Sex + Hypertension + Microvascular disease  <br/>
&check; Model8:ICU admission ~ Macrovascular disease + Age + Sex +Hypertension + Microvascular disease <br/>
&check; Model9:Death/ ICU admission~ Macrovascular disease+ Age + Sex +Hypertension + Microvascular disease <br/>



&check; When conducting Maximum Likelihood Estimation, it is assumed that the maximum likelihood estimate follows a normal distribution. 
&check; However, this may not be true in the case of small sample or sparse data.

&check; Since the standard errors of the general linear model are based on asymptotic variance, 
&check; they may not be a good estimator of standard error for small samples. 
&check; In particular, Wald Confidence Intervals may not perform very well. 
&check; One should only use the Wald Confidence Interval if the likelihood function is symmetric about the MLE.

&check; In cases where the likelihood function is not symmetric about the MLE, the Profile Likelihood Based Confidence Interval serves better. &check; This is because the Profile Likelihood Based Confidence Interval : 
&check; is based on the asymptotic chi-square distribution of the log likelihood ratio test statistic.

&check; More specifically, when using the Wald Confidence Interval,  
&check; two points on either side of MLE are chosen such that they are equidistant from MLE value 
&check; (MLE  ±  SE * (1-alpha)/2 percentile of Normal distribution). 

&check; However, when using the Profile Likelihood Based Confidence Interval, 
&check; two points on either side of MLE are chosen such that likelihood at those two points is equal to :
&check; (maximum likelihood – ½ * (1-alpha) percentile of the chi-square distribution with DF 1).
...
knitr::write_bib(c("stats","parameters","sjPlot"),file="references.bib")
...
>  Model4:Death ~ Macrovascular disease + Age + Sex + FX.COMBI(ACEi/ARB_only  Statin_only  Both_ACEi/ARB_and_Statin  Neiher_ACEI/ARB/Statin(basal)),     
   Model5:Death ~ Macrovascular disease + Age + Sex + FX.COMBI+HTA,    
   Model6:Death ~ Macrovascular disease + Age + Sex + FX.COMBI+HTA+CKD
   Model7:Death ~ Macrovascular disease + Age + Sex + Hypertension + Microvascular disease
   
```{r res1,include=TRUE, warning=F}

#Model7:Death ~ Macrovascular disease + Age + Sex + Hypertension + Microvascular disease  <br/>
#Model8:ICU admission ~ Macrovascular disease + Age + Sex +Hypertension + Microvascular disease <br/>
#Model9:Death/ ICU admission~ Macrovascular disease+ Age + Sex +Hypertension + Microvascular disease <br/>
...
c("ajuste15b1","ajuste15b2","ajuste15b4") %>% 
  map(~formula.text(.x,"Death2",taulavariables = conductor,dt=dades))%>% 
  map(~logistf(.x,data = dades, firth=FALSE)) %>% 
  sjPlot::tab_model(digits=5, 
                    show.intercept = FALSE,
                    dv.labels = c("Mortality Models (unadjusted)", "Mortality Models (adjusted)","Mortality Models (adjusted)"),
                    show.se = FALSE,
                    title = "Mortality models,Confidence Interval Profile likelihood ratio estimator, Chi-Square",
                    p.style = "numeric_stars")

print("Summary Model 1 ")


m00<-logistf(Death2 ~ DG.MACR,firth=FALSE,family=binomial,data = dades)
summary(m00)

print("Profile likelihood ratio estimator ,Model1:Mortality Models (unadjusted) ")
plot(profile(m00,variable="DG.MACRYes", legends = TRUE))


print("SE ,Model1:Mortality Models (unadjusted) ")
m1<-c("ajuste15b1") %>% formula.text("Death2",taulavariables = conductor,dt=dades)%>%glm(data = dades,family = "binomial") 
...
#print("Confidence Interval Profile likelihood ratio estimator, CHI-SQUARE")
#exp(confint(m1))
#exp(confint(m2))
#exp(confint(m3))

#print("Confidence Interval wald estimator, Z-NORMAL")
#exp(confint.default(m1))
#exp(confint.default(m2))
#exp(confint.default(m3))

#wald:
#CI_lower <- exp(coefficients(m1)[2] - 1.96*summary(m1)$coefficients[2,2])
#CI_upper <- exp(coefficients(m1)[2] + 1.96*summary(m1)$coefficients[2,2])
...
#m00<-logistf(Death2 ~ DG.MACR,firth=FALSE,family=binomial,data = dades)
#plot(profile(m00,variable="DG.MACRYes"))
#summary(m00)
...
#> summary(m00)
#logistf(formula = Death2 ~ DG.MACR, data = dades, firth = FALSE, 
#    family = binomial)

#Model fitted by Standard ML
#Coefficients:
#                  coef  se(coef) lower 0.95 upper 0.95      Chisq         p method
#(Intercept) -1.4399155 0.1447937 -1.7325390 -1.1638849        Inf 0.0000000      2
#DG.MACRYes   0.0789389 0.2894576 -0.5068843  0.6328652 0.07378973 0.7858968      2

#Method: 1-Wald, 2-Profile penalized log-likelihood, 3-None

#Likelihood ratio test=0.07378973 on 1 df, p=0.7858968, n=406
#Wald test = 0.07437245 on 1 df, p = 0.7850737

#Confidence Interval Profile likelihood ratio estimator, CHI-SQUARE
...
#
 ...
 #per a un interval de confiança del 95% i el posa a la trama. El 95%
 #L'interval de confiança de la probabilitat del perfil és el conjunt de punts
 #on es troba la funció de logaritme del perfil
 #per sobre del tall.
...
c("ajuste16a","ajuste16b","ajuste16c","ajuste16d") %>% 
  map(~formula.text(.x,"Death2",taulavariables = conductor,dt=dades))%>% 
  map(~logistf(.x,data = dades, firth=FALSE)) %>% 
  sjPlot::tab_model(digits=5, 
                    show.intercept = FALSE,
                    dv.labels = c("Mortality Models (adjusted)", "Mortality Models (adjusted)","Mortality Models (adjusted)","Mortality Models (adjusted)"),
                    show.se = FALSE,
                    title = "Mortality models,Confidence Interval Profile likelihood ratio estimator, Chi-Square",
                    p.style = "numeric_stars")
...
#  map(~formula.text(.x,"Death2",taulavariables = conductor,dt=dades))%>% 
#  map(~glm(.x,data = dades, family = "binomial")) %>% 
#  sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("Mortality Models (adjusted)", "Mortality Models (adjusted)","Mortality Models (adjusted)","Mortality Models (adjusted)"),
#                    show.se = FALSE,
#                    title = "Mortality models",
#                    p.style = "numeric_stars")
...
#I just discovered that someone answered this question in another post. 
#The answer is, confint uses profile confidence intervals, 
#whereas I was computing a Wald confidence interval (which can equivalently be computed using confint.default).

   
   
##Profile likelihood ratio! m1 
#m4<-c("ajuste16a") %>% formula.text("Death2",taulavariables = conductor,dt=dades)%>%glm(data = dades,family = "binomial") 
#m5<-c("ajuste16b") %>% formula.text("Death2",taulavariables = conductor,dt=dades)%>%glm(data = dades,family = "binomial") 
#m6<-c("ajuste16c") %>% formula.text("Death2",taulavariables = conductor,dt=dades)%>%glm(data = dades,family = "binomial") 
#m7<-c("ajuste16d") %>% formula.text("Death2",taulavariables = conductor,dt=dades)%>%glm(data = dades,family = "binomial") 
...
> Model1:ICU admission  ~ Macrovascular disease (unadjusted),     
  ...
  Model4:ICU admission  ~ Macrovascular disease + Age + Sex + Hypertension + Microvascular disease
...
c("ajuste15b1","ajuste15b2","ajuste15b4","ajuste15b5") %>% 
 map(~formula.text(.x,"Ad_UCI2",taulavariables = conductor,dt=dades))%>% 
  map(~logistf(.x,data = dades, firth=FALSE)) %>% 
  sjPlot::tab_model(digits=5, 
                    show.intercept = FALSE,
                    dv.labels = c("ICU admission Models (adjusted)", "ICU admission Models (adjusted)","ICU admission Models (adjusted)","ICU admission Models (adjusted)"),
                    show.se = FALSE,
                    title = "ICU admission models,Confidence Interval Profile likelihood ratio estimator, Chi-Square",
                    p.style = "numeric_stars")
...
#  map(~formula.text(.x,"Ad_UCI2",taulavariables = conductor,dt=dades))%>% 
#  map(~glm(.x,data = dades, family = "binomial")) %>% 
# sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("ICU admission Models (unadjusted)", "ICU admission Models (adjusted)","ICU admission models (adjusted)","ICU admission models (adjusted)"),
#                    show.se = FALSE,
#                    title = "ICU admission  models",
#                    p.style = "numeric_stars")
...
map(~formula.text(.x,"EV1.Mechanical_ventialtion2",taulavariables = conductor,dt=dades))%>% 
  ...
  sjPlot::tab_model(digits=5, 
                    show.intercept = FALSE,
                    dv.labels = c("Use of IMV on admission Models (unadjusted)", "Use of IMV on admission Models (adjusted)","Use of IMV on admission models (adjusted)"),
                    show.se = FALSE,
                    title = "Use of IMV on ICU admission models,Confidence Interval Profile likelihood ratio estimator, Chi-Square",
                    p.style = "numeric_stars")
...
>  Model1:Death/ ICU admission   ~ Macrovascular disease (unadjusted),     
   ...
   Model4:Death/ ICU admission  ~ Macrovascular disease + Age + Sex + Hypertension + Microvascular disease
...
c("ajuste15b1","ajuste15b2","ajuste15b4","ajuste15b5") %>% 
  ...
  sjPlot::tab_model(digits=5, 
                    show.intercept = FALSE,
                    dv.labels = c("Death/ICU admission  models (unadjusted)", "Death/ICU admission  models (adjusted)","Death/ICU admission  models (adjusted)","Death/ICU admission  models (adjusted)"),
                    show.se = FALSE,
                    title = "Death/ICU admission models,Confidence Interval Profile likelihood ratio estimator, Chi-Square",
                    p.style = "numeric_stars")
...
#  map(~formula.text(.x,"Death_DG_Ad_UCI2",taulavariables = conductor,dt=dades))%>% 
#  map(~glm(.x,data = dades, family = "binomial")) %>% 
# sjPlot::tab_model(digits=3, show.intercept = F,dv.labels = c("Death/ICU admission  models (unadjusted)", "Death/ICU admission  models (adjusted)","Death/ICU admission  models (adjusted)","Death/ICU admission  models (adjusted)"),
#                    show.se = FALSE,
#                    title = "Death/ICU admission  models",
#                    p.style = "numeric_stars")
